/**
 * @fileOverview wangdian2048 组件的安全适配器
...
KISSY.add(function (S, Gallery) {
    ...
    function init(frameGroup) {
         ...
         */
        function SafeWangdian2048(el, config) {
            this.inner = new Wangdian2048(el, config);//内部实际封装了真正的组件构造函数
        }

        //为我们门面模式包装的构造函数添加需要开放给外部使用的原型方法，on和destory只是举例，哪些需要暴露的方法就写哪些就行了
        SafeWangdian2048.prototype.on = function (type,fnc) {
            //this.inner.on(type,fnc);
        };

        //为我们门面模式包装的构造函数添加需要开放给外部使用的原型方法
        SafeWangdian2048.prototype.destory = function () {
            //this.inner.destory();
        };
        ...
        frameGroup.markCtor(SafeWangdian2048);

        frameGroup.grantMethod(SafeWangdian2048, "on");
        frameGroup.grantMethod(SafeWangdian2048, "destory");
        ...
        return function (context) {
            ...
            //最终需要返回给
            return {
                //这里的 wangdian2048 会自动放到KISSY.Wangdian2048:
            Wangdian2048: frameGroup.markFunction(function () {
                    return new SafeWangdian2048(arguments[0], cajaAFTB.untame(arguments[1]));
                }),
                kissy:true
            }
        }
